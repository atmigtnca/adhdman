...
def test_promote_inbox_item_to_task_creates_task_updates_inbox_and_logs_action(
    ...
    assert payload["source_inbox_item_id"] == inbox_item["id"]
    ...
    assert task_row == (payload["id"], "pay rent", "open", inbox_item["id"], None)
...
def test_promote_non_open_inbox_item_returns_409(tmp_path: Path, monkeypatch) -> None:
    ...
    with TestClient(app) as client:
        ...
        first_response = client.post(f"/inbox/{inbox_item['id']}/promote-task")
        second_response = client.post(f"/inbox/{inbox_item['id']}/promote-task")
